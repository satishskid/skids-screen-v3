...
import type { AnthropometryResult } from '../types'
...
function findNearestLMS(table: Record<number, LMSEntry>, key: number): LMSEntry {
  ...
  const fraction = (key - lower) / (upper - lower)
...
export function heightForAge(heightCm: number, ageMonths: number, gender: 'male' | 'female'): AnthropometryResult {
  ...
  return { value: heightCm, unit: 'cm', zScore: Math.round(z * 100) / 100, percentile: Math.round(zToPercentile(z) * 10) / 10, classification, referenceTable: ageMonths <= 60 ? 'WHO 2006' : 'WHO 2007' }
}

export function weightForAge(weightKg: number, ageMonths: number, gender: 'male' | 'female'): AnthropometryResult {
  ...
  return { value: weightKg, unit: 'kg', zScore: Math.round(z * 100) / 100, percentile: Math.round(zToPercentile(z) * 10) / 10, classification, referenceTable: ageMonths <= 60 ? 'WHO 2006' : 'WHO 2007' }
}

export function bmiForAge(weightKg: number, heightCm: number, ageMonths: number, gender: 'male' | 'female'): AnthropometryResult {
  ...
  return { value: Math.round(bmi * 10) / 10, unit: 'kg/m\u00B2', zScore: Math.round(z * 100) / 100, percentile: Math.round(zToPercentile(z) * 10) / 10, classification, referenceTable: ageMonths <= 60 ? 'WHO 2006' : 'WHO 2007' }
}

export function weightForHeight(weightKg: number, heightCm: number, gender: 'male' | 'female'): AnthropometryResult {
  const table = gender === 'male' ? WEIGHT_FOR_HEIGHT_BOYS : WEIGHT_FOR_HEIGHT_GIRLS
  const lms = findNearestLMS(table, heightCm)
  const z = calculateZScore(weightKg, lms)
  let classification = 'Normal'
  ...
  return { value: weightKg, unit: 'kg', zScore: Math.round(z * 100) / 100, percentile: Math.round(zToPercentile(z) * 10) / 10, classification, referenceTable: 'WHO 2006 WFH' }
}

export function classifyMUAC(muacMm: number): { label: string; color: string; severity: string; band: 'red' | 'yellow' | 'green' } {
  if (muacMm < 115) return { label: 'SAM (Severe Acute Malnutrition)', color: 'bg-red-50 border-red-300 text-red-700', severity: 'severe', band: 'red' }
  if (muacMm <= 125) return { label: 'MAM (Moderate Acute Malnutrition)', color: 'bg-yellow-50 border-yellow-300 text-yellow-700', severity: 'moderate', band: 'yellow' }
...
export function classifyAnemia(hbValue: number, ageMonths: number, gender: 'male' | 'female'): { label: string; color: string; severity: string } {
  ...
  if (hbValue >= normalThreshold) return { label: 'Normal', color: 'bg-green-50 border-green-300 text-green-700', severity: 'normal' }
  if (hbValue >= mildThreshold) return { label: 'Mild Anemia', color: 'bg-yellow-50 border-yellow-300 text-yellow-700', severity: 'mild' }
  if (hbValue >= moderateThreshold) return { label: 'Moderate Anemia', color: 'bg-orange-50 border-orange-300 text-orange-700', severity: 'moderate' }
